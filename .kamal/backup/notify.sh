...
# passes them separately (verified in /assets/functions/10-db-backup, tiredofit/db-backup 4.1.x): a
# failed S3 upload leaves $1=0, so checking only $1 would post a false "OK".
set -u
...
if [ "$exit_code" = "0" ] && [ "$move_exit_code" = "0" ]; then
  ...
  color=15548997
  hc_suffix="/fail"
  desc="DB: ${db_name}\\nDump exit: ${exit_code}  Upload exit: ${move_exit_code}\\nCheck: dotenv -f .env.production kamal backup-logs"
fi
