...
Dependencies are managed with **uv** (PEP 621 `[project]` table). `pyproject.toml` is the declaration and `uv.lock` pins the full resolved tree — both are committed; there is **no `requirements.txt`**. `[tool.uv] package = false` (this is an app, not a distributable package, so uv installs the dependencies but never the project root itself). Dev-only tools go in the `dev` dependency group (`[dependency-groups] dev = [...]`) and are excluded by `--no-dev`. **NOTE:** `pyproject.toml` declares only the direct dependencies, each pinned to an exact version (`==`). The transitive pins carried over from the Poetry migration have been dropped — `uv.lock` is the single source of truth for the resolved tree. Loosen the direct pins to ranges in a follow-up PR once test coverage exists; only then can `uv lock --upgrade` move them safely.
